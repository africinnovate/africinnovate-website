...
export async function POST(request: NextRequest) {
  const  { email,phoneNumber,fullName,selectCourse,yourMessage }  = await request.json()
...
//using the mailoptions to get the email and send the message
  const mailOptions: Mail.Options = {
    ...
    subject: `Message from (${email} with ${fullName} choosing ${selectCourse} and ${phoneNumber})`,
    text: yourMessage,
  }
